		...
		{#each projects as project}
			<Card cardAs="li">
				<div
					...
					<img src={project.logo} alt="" loading="lazy" width="32" height="32" decoding="async" class="h-8 w-8" style="color: transparent;" />
				</div>
				<h2 class="mt-6 text-base font-semibold text-zinc-800 dark:text-zinc-100">
					...
					<a href={project.link.href}>
						<span class="absolute -inset-x-4 -inset-y-6 z-20 sm:-inset-x-6 sm:rounded-2xl" /><span class="relative z-10">{project.name}</span>
					</a>
				</h2>
